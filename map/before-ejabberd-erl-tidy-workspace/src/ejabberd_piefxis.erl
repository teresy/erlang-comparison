...
-export([export_host/2, export_server/1,
	 import_file/1]).
...
-record(state,
	{xml_stream_state, user = <<"">>, server = <<"">>,
	 fd = self(), dir = <<"">>}).

-type({state, {type, 67, record, [{atom, 67, state}]},
       []}).
...
- spec ( { { import_file , 2 } , [ { type , 76 , 'fun' , [ { type , 76 , product , [ { type , 76 , binary , [ ] } , { type , 76 , state , [ ] } ] } , { type , 76 , union , [ { atom , 76 , ok } , { type , 76 , tuple , [ { atom , 76 , error } , { type , 76 , atom , [ ] } ] } ] } ] } ] } ) .
...
import_file(FileName, State) ->
    case file:open(FileName, [read, binary]) of
	  ...
	  Res = process(State#state{xml_stream_state =
					XMLStreamState,
				    fd = Fd, dir = Dir}),
	  file:close(Fd),
	  Res;
	  ...
	  ?ERROR_MSG("Failed to open file '~s': ~s",
		     [FileName, ErrTxt]),
	  {error, Reason}
    end.

- spec ( { { export_server , 1 } , [ { type , 93 , 'fun' , [ { type , 93 , product , [ { type , 93 , binary , [ ] } ] } , { type , 93 , any , [ ] } ] } ] } ) .


export_server(Dir) ->
    export_hosts(ejabberd_config:get_myhosts(), Dir).

- spec ( { { export_host , 2 } , [ { type , 97 , 'fun' , [ { type , 97 , product , [ { type , 97 , binary , [ ] } , { type , 97 , binary , [ ] } ] } , { type , 97 , any , [ ] } ] } ] } ) .


export_host(Dir, Host) -> export_hosts([Host], Dir).
...
export_host(Dir, FnH, Host) ->
    ...
    case file:open(DFn, [raw, write]) of
	  ...
	  case export_users(Users, Host, Fd) of
		...
		file:close(Fd),
		ok;
	    Err -> file:close(Fd), file:delete(DFn), Err
	  end;
      {error, Reason} ->
	  ErrTxt = file:format_error(Reason),
	  ?ERROR_MSG("Failed to open file '~s': ~s",
...
export_users([{User, _S} | Users], Server, Fd) ->
    case export_user(User, Server, Fd) of
      ok -> export_users(Users, Server, Fd);
      Err -> Err
    end;
export_users([], _Server, _Fd) -> ok.
...
export_user(User, Server, Fd) ->
    ...
    Els = get_offline(User, Server) ++
		...
		get_roster(User, Server) ++ get_private(User, Server),
    print(Fd,
	  fxml:element_to_binary(#xmlel{name = <<"user">>,
					attrs =
					    [{<<"name">>, User},
					     {<<"password">>, Pass}],
					children = Els})).

format_scram_password({StoredKey, ServerKey, Salt,
		       IterationCount}) ->
    StoredKeyB64 = base64:encode(StoredKey),
    ServerKeyB64 = base64:encode(ServerKey),
    SaltB64 = base64:encode(Salt),
    IterationCountBin = integer_to_binary(IterationCount),
    <<"scram:", StoredKeyB64/binary, ",",
      ServerKeyB64/binary, ",", SaltB64/binary, ",",
      IterationCountBin/binary>>.

parse_scram_password(PassData) ->
    Split = binary:split(PassData, <<",">>, [global]),
    [StoredKeyB64, ServerKeyB64, SaltB64,
     IterationCountBin] =
	Split,
    #scram{storedkey = StoredKeyB64,
	   serverkey = ServerKeyB64, salt = SaltB64,
	   iterationcount = binary_to_integer(IterationCountBin)}.

- spec ( { { get_vcard , 2 } , [ { type , 206 , 'fun' , [ { type , 206 , product , [ { type , 206 , binary , [ ] } , { type , 206 , binary , [ ] } ] } , { type , 206 , list , [ { type , 206 , xmlel , [ ] } ] } ] } ] } ) .
...
- spec ( { { get_offline , 2 } , [ { type , 215 , 'fun' , [ { type , 215 , product , [ { type , 215 , binary , [ ] } , { type , 215 , binary , [ ] } ] } , { type , 215 , list , [ { type , 215 , xmlel , [ ] } ] } ] } ] } ) .
...
- spec ( { { get_privacy , 2 } , [ { type , 227 , 'fun' , [ { type , 227 , product , [ { type , 227 , binary , [ ] } , { type , 227 , binary , [ ] } ] } , { type , 227 , list , [ { type , 227 , xmlel , [ ] } ] } ] } ] } ) .
...
get_roster(User, Server) ->
    ...
    case mod_roster:get_roster(User, Server) of
      [_ | _] = Items ->
	  Subs = lists:flatmap(fun (#roster{ask = Ask,
					    askmessage = Msg} =
					R)
				       ...
				       [xmpp:encode(#presence{from =
								  jid:make(R#roster.jid),
							      to = JID,
							      type = subscribe,
							      status =
								  xmpp:mk_text(Status)})];
			       ...
			       Items),
	  Rs = lists:flatmap(fun (#roster{ask = in,
					  subscription = none}) ->
				     [];
				 (R) -> [mod_roster:encode_item(R)]
			     end,
			     Items),
...
- spec ( { { get_private , 2 } , [ { type , 276 , 'fun' , [ { type , 276 , product , [ { type , 276 , binary , [ ] } , { type , 276 , binary , [ ] } ] } , { type , 276 , list , [ { type , 276 , xmlel , [ ] } ] } ] } ] } ) .


get_private(User, Server) ->
    case mod_private:get_data(User, Server) of
      [_ | _] = Els -> [xmpp:encode(#private{sub_els = Els})];
      _ -> []
    end.

process(#state{xml_stream_state = XMLStreamState,
	       fd = Fd} =
	    State) ->
    case file:read(Fd, ?CHUNK_SIZE) of
      {ok, Data} ->
	  NewXMLStreamState = fxml_stream:parse(XMLStreamState,
						Data),
	  case process_els(State#state{xml_stream_state =
					   NewXMLStreamState})
	      of
	    {ok, NewState} -> process(NewState);
	    Err -> fxml_stream:close(NewXMLStreamState), Err
	  end;
      eof -> fxml_stream:close(XMLStreamState), ok
    end.
...
process_els(State) ->
    receive
	  ...
	  case process_el(El, State) of
	    {ok, NewState} -> process_els(NewState);
	    Err -> Err
	  end
      after 0 -> {ok, State}
    end.

process_el({xmlstreamstart, <<"server-data">>, Attrs},
	   State) ->
    case fxml:get_attr_s(<<"xmlns">>, Attrs) of
      ?NS_PIEFXIS -> {ok, State};
      ?NS_PIE -> {ok, State};
      NS -> stop("Unknown 'server-data' namespace = ~s", [NS])
    end;
process_el({xmlstreamend, _}, State) -> {ok, State};
process_el({xmlstreamcdata, _}, State) -> {ok, State};
process_el({xmlstreamelement,
	    #xmlel{name = <<"xi:include">>, attrs = Attrs}},
	   #state{dir = Dir, user = <<"">>} = State) ->
    FileName = fxml:get_attr_s(<<"href">>, Attrs),
    case import_file(filename:join([Dir, FileName]), State)
	of
      ok -> {ok, State};
      Err -> Err
    end;
process_el({xmlstreamstart, <<"host">>, Attrs},
	   State) ->
    process_el({xmlstreamelement,
		#xmlel{name = <<"host">>, attrs = Attrs}},
	       State);
process_el({xmlstreamelement,
	    #xmlel{name = <<"host">>, attrs = Attrs,
		   children = Els}},
    ...
    try jid:decode(JIDS) of
	  ...
	  case ejabberd_router:is_my_host(S) of
	    true -> process_users(Els, State#state{server = S});
	    false -> stop("Unknown host: ~s", [S])
	  end
    catch
      _:{bad_jid, _} -> stop("Invalid 'jid': ~s", [JIDS])
    end;
process_el({xmlstreamstart, <<"user">>, Attrs},
	   State = #state{server = S})
    when S /= <<"">> ->
    process_el({xmlstreamelement,
		#xmlel{name = <<"user">>, attrs = Attrs}},
	       State);
process_el({xmlstreamelement,
	    #xmlel{name = <<"user">>} = El},
	   State = #state{server = S})
...
process_el({xmlstreamelement, El},
	   State = #state{server = S, user = U})
    when S /= <<"">>, U /= <<"">> ->
...
process_users([#xmlel{} = El | Els], State) ->
    case process_user(El, State) of
      {ok, NewState} -> process_users(Els, NewState);
      Err -> Err
    end;
process_users([_ | Els], State) ->
    process_users(Els, State);
process_users([], State) -> {ok, State}.

process_user(#xmlel{name = <<"user">>, attrs = Attrs,
		    children = Els},
    ...
    case jid:nodeprep(Name) of
      error -> stop("Invalid 'user': ~s", [Name]);
      LUser ->
	  case ejabberd_auth:try_register(LUser, LServer, Pass) of
	    ok -> process_user_els(Els, State#state{user = LUser});
...
process_user_els([#xmlel{} = El | Els], State) ->
    case process_user_el(El, State) of
      {ok, NewState} -> process_user_els(Els, NewState);
      Err -> Err
    end;
process_user_els([_ | Els], State) ->
    process_user_els(Els, State);
process_user_els([], State) -> {ok, State}.

process_user_el(#xmlel{name = Name, attrs = Attrs,
		       children = Els} =
		    El,
		State) ->
    try case {Name, fxml:get_attr_s(<<"xmlns">>, Attrs)} of
	  {<<"query">>, ?NS_ROSTER} ->
	  ...
	  {<<"presence">>, ?NS_CLIENT} ->
	      process_presence(xmpp:decode(El, ?NS_CLIENT,
					   [ignore_els]),
			       State);
	  _ -> {ok, State}
	end
...
- spec ( { { process_offline_msgs , 2 } , [ { type , 453 , 'fun' , [ { type , 453 , product , [ { type , 453 , list , [ { type , 453 , stanza , [ ] } ] } , { type , 453 , state , [ ] } ] } , { type , 453 , union , [ { type , 453 , tuple , [ { atom , 453 , ok } , { type , 453 , state , [ ] } ] } , { type , 453 , tuple , [ { atom , 453 , error } , { var , 453 , '_' } ] } ] } ] } ] } ) .


process_offline_msgs([#message{} = Msg | Msgs],
		     State) ->
    case process_offline_msg(Msg, State) of
      {ok, NewState} -> process_offline_msgs(Msgs, NewState);
      Err -> Err
    end;
process_offline_msgs([_ | Msgs], State) ->
    process_offline_msgs(Msgs, State);
process_offline_msgs([], State) -> {ok, State}.

- spec ( { { process_roster , 2 } , [ { type , 466 , 'fun' , [ { type , 466 , product , [ { type , 466 , roster_query , [ ] } , { type , 466 , state , [ ] } ] } , { type , 466 , union , [ { type , 466 , tuple , [ { atom , 466 , ok } , { type , 466 , state , [ ] } ] } , { type , 466 , tuple , [ { atom , 466 , error } , { var , 466 , '_' } ] } ] } ] } ] } ) .


process_roster(RosterQuery,
	       State = #state{user = U, server = S}) ->
    case mod_roster:set_items(U, S, RosterQuery) of
      {atomic, _} -> {ok, State};
      Err -> stop("Failed to write roster: ~p", [Err])
    end.

- spec ( { { process_privacy , 2 } , [ { type , 475 , 'fun' , [ { type , 475 , product , [ { type , 475 , privacy_query , [ ] } , { type , 475 , state , [ ] } ] } , { type , 475 , union , [ { type , 475 , tuple , [ { atom , 475 , ok } , { type , 475 , state , [ ] } ] } , { type , 475 , tuple , [ { atom , 475 , error } , { var , 475 , '_' } ] } ] } ] } ] } ) .
...
process_privacy(#privacy_query{lists = Lists,
    ...
    if Default /= undefined ->
	   process_privacy2(JID,
			    #privacy_query{default = Default});
       true -> ok
    end,
    {ok, State}.

process_privacy2(JID, PQ) ->
    case mod_privacy:process_iq(#iq{type = set,
				    id = p1_rand:get_string(), from = JID,
				    to = JID, sub_els = [PQ]})
	of
      #iq{type = error} = ResIQ ->
...
- spec ( { { process_private , 2 } , [ { type , 515 , 'fun' , [ { type , 515 , product , [ { type , 515 , private , [ ] } , { type , 515 , state , [ ] } ] } , { type , 515 , union , [ { type , 515 , tuple , [ { atom , 515 , ok } , { type , 515 , state , [ ] } ] } , { type , 515 , tuple , [ { atom , 515 , error } , { var , 515 , '_' } ] } ] } ] } ] } ) .


process_private(Private,
		State = #state{user = U, server = S}) ->
    JID = jid:make(U, S),
    IQ = #iq{type = set, id = p1_rand:get_string(),
	     from = JID, to = JID, sub_els = [Private]},
    case mod_private:process_sm_iq(IQ) of
      #iq{type = result} -> {ok, State};
      Err -> stop("Failed to write private: ~p", [Err])
    end.

- spec ( { { process_vcard , 2 } , [ { type , 527 , 'fun' , [ { type , 527 , product , [ { type , 527 , xmlel , [ ] } , { type , 527 , state , [ ] } ] } , { type , 527 , union , [ { type , 527 , tuple , [ { atom , 527 , ok } , { type , 527 , state , [ ] } ] } , { type , 527 , tuple , [ { atom , 527 , error } , { var , 527 , '_' } ] } ] } ] } ] } ) .
...
process_vcard(El,
    ...
    case mod_vcard:process_sm_iq(IQ) of
      #iq{type = result} -> {ok, State};
      Err -> stop("Failed to write vcard: ~p", [Err])
    end.

- spec ( { { process_offline_msg , 2 } , [ { type , 539 , 'fun' , [ { type , 539 , product , [ { type , 539 , message , [ ] } , { type , 539 , state , [ ] } ] } , { type , 539 , union , [ { type , 539 , tuple , [ { atom , 539 , ok } , { type , 539 , state , [ ] } ] } , { type , 539 , tuple , [ { atom , 539 , error } , { var , 539 , '_' } ] } ] } ] } ] } ) .


process_offline_msg(#message{from = undefined},
		    _State) ->
    stop("No 'from' attribute found", []);
process_offline_msg(Msg,
		    State = #state{user = U, server = S}) ->
    To = jid:make(U, S),
    ejabberd_hooks:run_fold(offline_message_hook,
			    To#jid.lserver, {pass, xmpp:set_to(Msg, To)}, []),
    {ok, State}.

- spec ( { { process_presence , 2 } , [ { type , 548 , 'fun' , [ { type , 548 , product , [ { type , 548 , presence , [ ] } , { type , 548 , state , [ ] } ] } , { type , 548 , union , [ { type , 548 , tuple , [ { atom , 548 , ok } , { type , 548 , state , [ ] } ] } , { type , 548 , tuple , [ { atom , 548 , error } , { var , 548 , '_' } ] } ] } ] } ] } ) .


process_presence(#presence{from = undefined}, _State) ->
    stop("No 'from' attribute found", []);
process_presence(Pres,
		 #state{user = U, server = S} = State) ->
    To = jid:make(U, S),
...
stop(Fmt, Args) ->
    ?ERROR_MSG(Fmt, Args), {error, import_failed}.
...
%% @spec () -> string()
make_piefxis_server_tail() -> "</server-data>".
...
print(Fd, String) -> file:write(Fd, String).
