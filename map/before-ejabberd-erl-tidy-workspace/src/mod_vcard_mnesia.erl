...
%% API
-export([get_vcard/2, import/3, init/2, remove_user/2,
	 search/4, search_fields/1, search_reported/1,
	 set_vcard/4, stop/1]).
...
init(_Host, _Opts) ->
    ...
    ejabberd_mnesia:create(?MODULE, vcard_search,
			   [{disc_copies, [node()]},
			    ...
			    {index,
			     [luser, lfn, lfamily, lgiven, lmiddle, lnickname,
			      lbday, lctry, llocality, lemail, lorgname,
			      lorgunit]}]).

stop(_Host) -> ok.

is_search_supported(_ServerHost) -> true.
...
import(LServer, <<"vcard">>,
       [LUser, XML, _TimeStamp]) ->
...
import(LServer, <<"vcard_search">>,
       [User, LUser, FN, LFN, Family, LFamily, Given, LGiven,
	Middle, LMiddle, Nickname, LNickname, BDay, LBDay, CTRY,
	LCTRY, Locality, LLocality, EMail, LEMail, OrgName,
	LOrgName, OrgUnit, LOrgUnit]) ->
    mnesia:dirty_write(#vcard_search{us = {LUser, LServer},
...
need_transform(#vcard{us = {U, S}})
    when is_list(U) orelse is_list(S) ->
    ?INFO_MSG("Mnesia table 'vcard' will be converted "
	      "to binary",
	      []),
    true;
need_transform(#vcard_search{us = {U, S}})
    when is_list(U) orelse is_list(S) ->
    ?INFO_MSG("Mnesia table 'vcard_search' will be "
	      "converted to binary",
	      []),
    true;
need_transform(_) -> false.
...
filter_fields([], Match, _LServer) -> Match;
...
mod_options(_) -> [{search_all_hosts, true}].
