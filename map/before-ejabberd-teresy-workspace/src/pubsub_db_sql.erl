...
-export([add_subscription/1, delete_subscription/1,
	 read_subscription/1, update_subscription/1]).
...
subscription_opt_from_sql([<<"SHOW_VALUES">>, Value]) ->
    {show_values, Value};
subscription_opt_from_sql([<<"SUBSCRIPTION_TYPE">>,
			   Value]) ->
    {subscription_type,
     case Value of
       <<"items">> -> items;
...
subscription_opt_from_sql([<<"SUBSCRIPTION_DEPTH">>,
			   Value]) ->
    {subscription_depth,
     case Value of
       <<"all">> -> all;
	   ...
	   #pubsub_node{nodeid = {Host, Node}, id = Nidx,
	      ...
	      [?SQL("delete from pubsub_node where nodeid=%(Nidx)d;"),
	       ?SQL("delete from pubsub_node_option where "
		    "nodeid=%(Nidx)d;"),
	       ?SQL("delete from pubsub_node_owner where "
		    "nodeid=%(Nidx)d;"),
     ...
     {pubsub_state,
      fun (_Host,
	   #pubsub_state{stateid = {JID, Nidx},
	      ...
	      [?SQL("insert into pubsub_state(nodeid,jid,affiliati"
		    "on,subscriptions) values (%(Nidx)d, "
		    "%(J)s, %(A)s, %(S)s);")];
	  (_Host, _R) -> []
      end},
     {pubsub_item,
      fun (_Host,
	   #pubsub_item{itemid = {ItemId, Nidx}, creation = {C, _},
			modification = {M, JID}, payload = Payload}) ->
	      P = jid:encode(JID),
	      XML = str:join([fxml:element_to_binary(X)
			      || X <- Payload],
			     <<>>),
	      SM = encode_now(M),
	      SC = encode_now(C),
	      [?SQL("insert into pubsub_item(itemid,nodeid,creatio"
		    "n,modification,publisher,payload) values "
		    "(%(ItemId)s, %(Nidx)d, %(SC)s, %(SM)s, "
		    "%(P)s, %(XML)s);")];
	  (_Host, _R) -> []
      end}].
