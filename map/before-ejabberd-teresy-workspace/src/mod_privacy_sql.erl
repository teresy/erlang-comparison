...
%% API
-export([export/1, get_list/3, get_lists/2, import/1,
	 init/2, remove_list/3, remove_lists/2, set_default/3,
	 set_list/4, set_lists/1, unset_default/2]).
    ...
    F = fun () ->
		case get_privacy_list_names_t(LUser, LServer) of
		  ...
		  {selected, Names} ->
			...
			true -> set_default_privacy_list(LUser, LServer, Name);
...
remove_list(LUser, LServer, Name) ->
    F = fun () ->
		case get_default_privacy_list_t(LUser, LServer) of
		  ...
		  {selected, [{Default}]} ->
		      if Name == Default -> {error, conflict};
			 true -> remove_privacy_list_t(LUser, LServer, Name)
		      end
		end
	end,
    transaction(LServer, F).
...
set_list(LUser, LServer, Name, List) ->
    ...
    F = fun () ->
		ID = case get_privacy_list_id_t(LUser, LServer, Name) of
		       {selected, []} ->
			   add_privacy_list(LUser, LServer, Name),
			   {selected, [{I}]} = get_privacy_list_id_t(LUser,
								     LServer,
								     Name),
			   I;
		       {selected, [{I}]} -> I
...
get_lists(LUser, LServer) ->
    case get_default_privacy_list(LUser, LServer) of
	  ...
	  case get_privacy_list_names(LUser, LServer) of
	    {selected, Names} ->
		case lists:foldl(fun (_, {error, _} = Err) -> Err;
				     ({Name}, Acc) ->
					 case get_privacy_list_data(LUser,
								    LServer,
								    Name)
					     of
					   {selected, RItems} ->
					       Items =
						   lists:flatmap(fun raw_to_item/1,
								 RItems),
					       [{Name, Items} | Acc];
					   _Err -> {error, db_failure}
					 end
		  ...
		  Lists ->
		      {ok,
		       #privacy{default = Default, us = {LUser, LServer},
				lists = Lists}}
		end;
	    _Err -> {error, db_failure}
	  end;
	   ...
	   #privacy{us = {LUser, LServer}, lists = Lists,
		...
		lists:flatmap(fun ({Name, List}) ->
					...
					[?SQL("insert into privacy_list_data(id, t, "
					      ...
					      "nce_out) values (%(ID)d, %(SType)s, "
					      "%(SValue)s, %(SAction)s, %(Order)d, "
					      "%(MatchAll)b, %(MatchIQ)b, %(MatchMessage)b, "
					      "%(MatchPresenceIn)b, %(MatchPresenceOut)b);")
					 || {SType, SValue, SAction, Order,
					     MatchAll, MatchIQ, MatchMessage,
					     MatchPresenceIn, MatchPresenceOut}
						<- RItems]
			      end,
			      Lists);
	  (_Host, _R) -> []
...
get_id() -> ID = get(id), put(id, ID + 1), ID + 1.
...
raw_to_item({SType, SValue, SAction, Order, MatchAll,
	     ...
	     MatchPresenceOut} =
		Row) ->
    try {Type, Value} = case SType of
			  <<"n">> -> {none, none};
			  <<"j">> ->
			      JID = jid:decode(SValue), {jid, jid:tolower(JID)};
			  <<"g">> -> {group, SValue};
			  <<"s">> ->
			      case SValue of
...
get_default_privacy_list_t(LUser, LServer) ->
    ejabberd_sql:sql_query_t(?SQL("select @(name)s from privacy_default_list "
				  "where username=%(LUser)s and %(LServer)H")).

get_privacy_list_names(LUser, LServer) ->
    ejabberd_sql:sql_query(LServer,
			   ?SQL("select @(name)s from privacy_list where "
				"username=%(LUser)s and %(LServer)H")).

get_privacy_list_names_t(LUser, LServer) ->
    ejabberd_sql:sql_query_t(?SQL("select @(name)s from privacy_list where "
				  "username=%(LUser)s and %(LServer)H")).

get_privacy_list_id_t(LUser, LServer, Name) ->
    ejabberd_sql:sql_query_t(?SQL("select @(id)d from privacy_list where "
				  "username=%(LUser)s and %(LServer)H and "
				  "name=%(Name)s")).

get_privacy_list_data(LUser, LServer, Name) ->
    ejabberd_sql:sql_query(LServer,
			   ?SQL("select @(t)s, @(value)s, @(action)s, "
				"@(ord)d, @(match_all)b, @(match_iq)b, "
				"@(match_message)b, @(match_presence_in)b, "
				"@(match_presence_out)b from privacy_list_data "
				"where id = (select id from privacy_list "
				"where username=%(LUser)s and %(LServer)H "
				"and name=%(Name)s) order by ord")).

set_default_privacy_list(LUser, LServer, Name) ->
    ?SQL_UPSERT_T("privacy_default_list",
		  ["!username=%(LUser)s", "!server_host=%(LServer)s",
		   "name=%(Name)s"]).
...
remove_privacy_list_t(LUser, LServer, Name) ->
    case
      ejabberd_sql:sql_query_t(?SQL("delete from privacy_list where username=%(LUs"
				    "er)s and %(LServer)H and name=%(Name)s"))
	of
...
add_privacy_list(LUser, LServer, Name) ->
    ejabberd_sql:sql_query_t(?SQL_INSERT("privacy_list",
					 ["username=%(LUser)s",
					  "server_host=%(LServer)s",
					  "name=%(Name)s"])).

set_privacy_list(ID, RItems) ->
    ejabberd_sql:sql_query_t(?SQL("delete from privacy_list_data where "
				  "id=%(ID)d")),
    lists:foreach(fun ({SType, SValue, SAction, Order,
			MatchAll, MatchIQ, MatchMessage, MatchPresenceIn,
			MatchPresenceOut}) ->
			  ejabberd_sql:sql_query_t(?SQL("insert into privacy_list_data(id, t, "
							"value, action, ord, match_all, match_iq, "
							"match_message, match_presence_in, match_prese"
							"nce_out) values (%(ID)d, %(SType)s, "
							"%(SValue)s, %(SAction)s, %(Order)d, "
